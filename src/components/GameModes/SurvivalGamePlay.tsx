...
import { Clock, Users, Trophy, Zap, X, Heart, Skull } from 'lucide-react'
...
interface SurvivalGamePlayProps {
  onGameEnd: (results: any) => void
  onLeaveGame?: () => void
}

const SurvivalGamePlay: React.FC<SurvivalGamePlayProps> = ({ onGameEnd, onLeaveGame }) => {
  ...
  useEffect(() => {
    if (currentQuestion && !answered && timeLeft > 0) {
      ...
      // Auto-submit when time runs out (wrong answer in survival)
      handleSubmitAnswer()
    }
  ...
  useEffect(() => {
    if (currentQuestion) {
      setTimeLeft(currentQuestion.timeLimit || 25)
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      const timeSpent = (currentQuestion.timeLimit || 25) - timeLeft
      ...
      if (result && typeof result === 'object' && 'isCorrect' in result) {
        if (result.isCorrect) {
          ...
          setCurrentStreak(0)
          // In survival mode, losing a life is handled by the server
        }
      }
    } catch (error) {
  ...
  const handleCreateSurvivalChallenge = async () => {
    if (!gameState?.roomCode || !isHost) return

    try {
      // Create contextual survival micro-challenge
          ...
          <div className="mb-6">
            ...
            <div className="space-y-3">
              {currentQuestionResults.map((result, index) => {
                const playerLives = gameState.players?.find(p => p.user.id === result.userId)?.lives || 0
                const lostLife = !result.isCorrect
          ...
          {isHost && (
            <div className="flex justify-center gap-3">
              <motion.button
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                onClick={handleCreateSurvivalChallenge}
  ...
  const lives = currentPlayer?.lives || 3
  const maxLives = 3
  ...
  return (
    <div className="min-h-screen bg-slate-950 relative overflow-hidden">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 -left-20 w-72 h-72 bg-gradient-to-r from-red-600/10 to-orange-600/10 rounded-full blur-3xl"></div>
        <div className="absolute -top-20 right-1/4 w-72 h-72 bg-gradient-to-r from-purple-600/10 to-pink-600/10 rounded-full blur-3xl"></div>
      </div>

      <div className="relative min-h-screen flex flex-col">
        {/* Header with Survival Mode Info */}
        <div className="p-6">
            ...
            <div className="flex items-center justify-between mt-4">
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-xl font-mono ${timeLeft <= 5 ? 'text-red-400' : 'text-slate-200'}`}>
                    {timeLeft}s
                  </span>
                </div>
              ...
              <div className="flex items-center gap-4">
                ...
                {gameState.players?.length > 1 && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-slate-300">{gameState.players.length} survivants</span>
                  </div>
                )}
              </div>
            ...
            <div className="mb-8">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-slate-400 text-sm uppercase tracking-wider">
                  {currentQuestion.category || 'General'} • Mode Survie
                </span>
              </div>
                ...
                <div className="flex gap-2 overflow-x-auto">
                  {gameState.players?.map((player) => {
                    ...
                    return (
                      <div
                        key={player.id}
                        className={`flex-shrink-0 px-3 py-2 rounded-lg text-xs font-medium flex items-center gap-2 ${
                          isEliminated
...
export default SurvivalGamePlay
